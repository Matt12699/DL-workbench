...
def encoderClassifier_train(processed_csv_path: str, dataset_name: str, positive_label_value: int, plots_dir_train: str , early_stopping_metric: str, model_config: str, encoder_path:str):
    ...
    plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.legend(); plt.grid(True)
    plt.savefig(os.path.join(plots_dir_train, f'self_sup_Classifier_loss_curve_{dataset_name}.png'))
    plt.close()
    logging.info(f"Loss curve plot saved to {os.path.join(plots_dir_train, f'self_sup_Classifier_loss_curve_{dataset_name}.png')}")
    ...
    plt.close()
    logging.info(f"Validation metrics curve plot saved to {os.path.join(plots_dir_train, f'self_sup_validation_metrics_curve_{dataset_name}.png')}")
...
def self_supervised_evaluate_model(encoder_path: str, model_path: str, processed_csv_path: str, dataset_name: str, positive_label_value: int, plots_dir: str):
    ...
    # Salvo il grafico
    plot_filename = f"Self_sup_confusion_matrix_{dataset_name}.png"
    ...
    plot_metrics_filename = f"Self_sup_metrics_barchart_{dataset_name}.png"
    ...
    plt.legend()
    plt.grid(True)
    plot_pr_filename = f"Self_sup_precision_recall_curve_{dataset_name}.png"
